...
Cloud = function () {
	...
	// Lấy kích thước hình ảnh và tạo mặt phẳng với tỷ lệ đúng
	getImageSize(imagePath, (width, height) => {
		var aspectRatio = width / height;
		var geom = new THREE.PlaneGeometry(15 * aspectRatio, 15); // Thay đổi kích thước mặt phẳng dựa trên tỉ lệ ảnh

		// Tạo kết cấu từ hình ảnh
		var texture = new THREE.TextureLoader().load(imagePath);

		// Tạo vật liệu với kết cấu hình ảnh
		var mat = new THREE.MeshBasicMaterial({
			map: texture,
			side: THREE.DoubleSide
		});

		// Tạo bản sao của mặt phẳng
		var m = new THREE.Mesh(geom, mat);

		// Vị trí ngẫu nhiên cho mỗi mặt phẳng
		m.position.x = 100;
		m.position.y = 0;
...
function loop() {
	land.mesh.rotation.z += .0025;
	orbit.mesh.rotation.z += .0005;
	sky.mesh.rotation.z += .0015;
	forest.mesh.rotation.z += .0025;
	updatePlane();
